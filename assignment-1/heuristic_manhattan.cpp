...
//calculate heuristic
int calculate_manhattan_distance(node* n)
{
    int distance = 0;
    int dim = sqrt(no_of_elements);
    ...
    {
        if(n->orientation[i] != 0) // Skip the empty tile
        {
            int current_row = i / dim;
            int current_col = i % dim;
            int goal_row = goal_test[n->orientation[i]] / dim;
            int goal_col = goal_test[n->orientation[i]] % dim;
...
node* create_successor_node(node* state, int pos1, int pos2)
{
    ...
    new_state->h_cost = calculate_manhattan_distance(new_state);
...
{
    n->h_cost = calculate_manhattan_distance(n);
    n->f_cost = n->h_cost;
    n->parent = NULL;
